...
u32 pinSet[2][8] = {
    ...
    if (On) {
		if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_S5K5E2YA_MIPI_RAW,currSensorName)))
        {
           ...
           if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1200/*VOL_1800*/,mode_name))
		   {
			   PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
			   //return -EIO;
			   goto _kdCISModulePowerOn_exit_;
		...
		else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_BF3905_MIPI_YUV,currSensorName)) && (pinSetIdx == 1))
       {
		   if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D2, VOL_1800,mode_name))
		   {
			   PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");//2.8 ->1.8
